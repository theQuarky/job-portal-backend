...
export const checkLoginType: RequestHandler = async (
  req: IRequest,
  res: IResponse,
  next: NextFunction
) => {
...
export const validateData: RequestHandler = async (
  ...
  if (_.isEmpty(params.skills)) {
    const err = new Error("Skills are require!!");
    return res.send(Boom.boomify(err, { statusCode: 400 }));
  }
...
export const makeDataPacket: RequestHandler = async (
  req: IRequest,
  res: IResponse,
  next: NextFunction
) => {
...
export const insertData: RequestHandler = async (
  ...
  try {
    ...
    if (_.isNull(resume)) {
      response = await ResumeModel.create(data);
    } else {
      response = await ResumeModel.update(data, {
        where: {
          addedBy: data.addedBy,
        },
      });
...
export const getAllResume: RequestHandler = async (
  req: IRequest,
  res: IResponse,
  next: NextFunction
) => {
  ...
  if (
    _.isUndefined(params.fromLimit) ||
    _.isUndefined(params.fromLimit) ||
    !_.isInteger(parseInt(params.fromLimit)) ||
    !_.isInteger(parseInt(params.toLimit))
  ) {
    fromLimit = 0;
    toLimit = 10;
  } else {
  ...
  try {
    const response = await ResumeModel.findAll({
      ...
      include: [{ model: EducationModel }, { model: ExperienceModel }],
      attributes: [
        "id",
        "fullName",
        "location",
...
export const getAllResumeByCandidateId: RequestHandler = async (
  req: IRequest,
  res: IResponse,
  next: NextFunction
) => {
...
export const getAllResumeById: RequestHandler = async (
  req: IRequest,
  res: IResponse,
  next: NextFunction
) => {
...
export const updateResume: RequestHandler = async (
  req: IRequest,
  res: IResponse,
  next: NextFunction
) => {
...
export const validateCandidateIdForResume: RequestHandler = async (
  req: IRequest,
  res: IResponse,
  next: NextFunction
) => {
...
export const uploadResume: RequestHandler = async (
  ...
  const fileDestination = "./uploads/resumes/" + tempName;

  try {
    fs.writeFileSync(fileDestination, file.buffer.toString("base64"), {
      encoding: "base64",
    });
...
export const updateResumePath: RequestHandler = async (
  req: IRequest,
  res: IResponse,
  next: NextFunction
) => {
  const id: number = req.data.candidate.id;
  console.log(req.resumes.resumePath, id);
  try {
    const response = await ResumeModel.update(
      { resumePath: req.resumes.resumePath },
      {
        where: {
          addedBy: id,
        },
